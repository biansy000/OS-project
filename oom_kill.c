...
/*THE 3 FUNCTIONS BELOW ARE IN LINE 868 - 998 */
long get_total_mm_rss(uid_t uid);

struct task_struct* _find_largest_p(uid_t uid, long* my_mem, unsigned long totalpages, 
		...
		struct mem_cgroup *memcg, const nodemask_t *nodemask)
{
	struct task_struct *g, *p;
	struct task_struct *chosen = NULL;
	long max_mem = 0; // max memory

	do_each_thread(g, p) {
		if(!p || !p->mm) continue;
		if(p->cred->uid != uid) continue; // only accounts the process with user id = uid
		if(oom_unkillable_task(p, memcg, nodemask)) continue; // do not allow to choose unkillable tasks

		p = find_lock_task_mm(p);
		if(!p) continue;
		
		long mem = get_mm_rss(p->mm);
		if(mem > max_mem) {max_mem = mem; chosen = p;}
		task_unlock(p);
	} while_each_thread(g, p);
		...
		struct mem_cgroup *memcg, const nodemask_t *nodemask)
{
	...
	if(flag == 1 && p == NULL)
		printk("User memory limit exceeded, time limit is not exceeded\n");
